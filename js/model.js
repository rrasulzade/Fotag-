...
function createModelModule() {
    ...
    var IMAGE_ADDED = 'IMAGE_ADDED';
    var IMAGE_CHANGED = 'IMAGE_CHANGED';
    var IMAGE_REMOVED = 'IMAGE_REMOVED';
    /**
    ...
    _.extend(ImageModel.prototype, {
        ...
        setCaption: function(caption) {
            ...
            if (this.caption != caption  && this.curState != IMAGE_REMOVED) {
                this.caption = caption;
                this.modification = new Date();
                this.curState = IMAGE_CHANGED;
                var self = this;

                _.each(
                    this.listeners,
                    function(listener_fn) {
                        listener_fn(self, self.curState, new Date());
                    }
                    );
            }
        ...
        setID: function(){
            var name = this.getPath().substring(7);
            var noJPG = name.substr(0, name.length-4);
            this.idNum = noJPG + global_ID;
            global_ID++;
        },

        deleteImage: function(){
            var self = this;
            this.curState = IMAGE_REMOVED;
            _.each(
                this.listeners,
                function(listener_fn) {
                    listener_fn(self, self.curState, new Date());
    ...
    _.extend(ImageCollectionModel.prototype, {
        ...
        addImageModel: function(imageModel) {
            ...
            imageModel.addListener(function(imageModel, eventType, eventDate){
                if (eventType === IMAGE_CHANGED){
                    _.each( self.listeners,
                            function(listener_fn) {
                                listener_fn(IMAGE_META_DATA_CHANGED_EVENT, self, imageModel, new Date())
                            }
                    );
                }else if (eventType === IMAGE_REMOVED) {
                    _.each(
                        self.listeners,
                        function(listener_fn) {
